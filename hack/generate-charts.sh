...
sed_i "s|tag: \"latest\"|tag: \"${AGENTCUBE_IMAGE_TAG}\"|g" $(find ${CHART_OUT_PATH} -type f | grep values.yaml)
sed_i "s|version: \"latest\"|version: \"${AGENTCUBE_CHART_VERSION}\"|g" $(find ${CHART_OUT_PATH} -type f | grep Chart.yaml)
sed_i "s|appVersion: \"latest\"|appVersion: \"${AGENTCUBE_CHART_VERSION}\"|g" $(find ${CHART_OUT_PATH} -type f | grep Chart.yaml)
